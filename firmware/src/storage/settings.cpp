...
void applyLine(char* line, Settings* out) {
    ...
    *eq = '\0';
    ...
    } else if (strcmp(key, "backlight_dim_seconds") == 0) {
        int parsed = atoi(value);
        if (parsed > 0) out->backlightDimSeconds = parsed;
    } else if (strcmp(key, "backlight_off_seconds") == 0) {
        int parsed = atoi(value);
        if (parsed > 0) out->backlightOffSeconds = parsed;
    }
...
bool saveSettings(const Settings& settings) {
    ...
    int len = snprintf(buf, sizeof(buf),
                        ...
                        "backlight_dim_seconds=%d\n"
                        "backlight_off_seconds=%d\n",
                        ...
                        settings.syncIntervalSeconds, settings.backlightDimSeconds, settings.backlightOffSeconds);
