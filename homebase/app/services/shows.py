...
def create_show(db: Session, name: str, event_date: date, location: str) -> Show:
    show = Show(name=name.strip(), event_date=event_date, location=location.strip())
    db.add(show)
    db.commit()
    db.refresh(show)

    # First show ever created becomes active automatically — otherwise the
    # rest of the app (Cars/Classes/Criteria) has nowhere to point and every
    # one of those pages would just redirect back here anyway.
    settings = _get_or_create_settings(db)
    if settings.active_show_id is None:
        settings.active_show_id = show.id
        db.commit()

    return show


def update_show(db: Session, show_id: int, name: str, event_date: date, location: str) -> Show:
    show = db.get(Show, show_id)
    if show is None:
        raise ValueError(f"No show with id {show_id}")
