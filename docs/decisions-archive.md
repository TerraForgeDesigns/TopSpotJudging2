# Decisions Archive — Full Rationale

Full-length reasoning for every entry in [DECISIONS.md](../DECISIONS.md)'s Decided
section, preserved verbatim and in the same order it was originally written, so a
one-line summary there can link back to the complete "why" here. DECISIONS.md itself
now carries only a one-line, dated summary per decision plus active open items — see
its own header and "Notes for future sessions" for the split. Nothing here is
current-truth-of-record on its own; where content and the actual codebase disagree,
the codebase (and DECISIONS.md's own struck-through corrections) win.

New rationale for a NEW decision goes here going forward, appended in date order,
with a matching one-line summary added to DECISIONS.md — never the other way around.
...
<a id="d71"></a>
- **SIM1 — Handheld simulator: a browser reproduction of the ESP32 handheld's
  screens/theme/interaction model, served by Home Base itself, talking to the
  real sync API.** Research before designing surfaced that the task's own premise
  depended on something that doesn't exist — see point 1.
  1. **"The simulator form of the master vehicle data generated by HB5's
     preparation script" doesn't exist — HB5 was never built.** Confirmed by
     reading the codebase directly: Home Base has no `VehicleMake`/`VehicleModel`
     tables, no preparation script, no export. The only HB5-adjacent code is
     `vehicle_candidates.py`, which just records sightings for a future review
     queue. Surfaced to the user directly (via a clarifying question) rather than
     silently inventing a second vehicle list or building HB5 for real (well
     outside this task's scope). The user chose: export the firmware's own real,
     already-sourced list instead. `firmware/tools/export_vehicle_data_for_
     simulator.py` (new) reads `vehicle_seed_source.json` — the same
     vPIC-plus-curated-classic, provenance-tagged data F4 already flashes to the
     real device — and writes `homebase/app/static/simulator/data/
     vehicle_data.json`. Ran clean: 423 makes, 9,176 models, matching F4's real
     counts exactly (confirms pass-through correctness, not a coincidence).
     Full HB5 (review queue, learned-vocabulary approval) stays out of scope,
     documented plainly rather than glossed over — `vehicle_additions[]` in
     `sync.js` is wired but genuinely empty from real Home Base today, same
     "sits honestly inactive until the gap closes" pattern as F4/F5's own
     handling of that field.
  2. **Served BY Home Base itself, not a standalone server.** A new route
     (`GET /simulator`, `app/web/simulator.py`) follows the exact existing
     per-feature-router pattern (`base_context()`/`require_active_show()`, same
     redirect-to-`/shows` rule every other page uses). Same-origin means the
     simulator calls the real `POST /api/v1/sync` directly with zero CORS setup
     — verified for real, not just read: stood up a live Home Base instance, drove
     the actual wizard service functions to materialize a genuine show, and hit
     `/api/v1/sync` with the simulator's exact request/response field names.
     Confirmed the full response shape (`config_revision`, `data_revision`,
     `configuration.*`, `cars[]`, `vehicle_additions`, `results[]`, `summary.*`)
     matches `sync.js`'s `applyHit()` field-for-field, and that `SubmissionIn`
     (`app/api/schemas.py`) matches `review.js`'s `queuedCar` object
     field-for-field — built against the real schema, not PROTOCOL.md's example
     JSON (confirmed during planning that PROTOCOL.md's example includes fields
     `ConfigurationOut` doesn't actually have — a pre-existing doc/code gap, not
     something this task needed to fix).
  3. **Found and fixed a real, pre-existing Home Base bug while verifying the
     range-escalation trigger end-to-end**: `services/cars.py::add_cars()` added
     new `Car` rows via `db.add()` then immediately called `check_and_escalate()`,
     which counts cars with a fresh query — but the session has `autoflush=False`
     (`app/db.py`), so that count query never saw the rows just added in the same
     transaction. Adding 150+ cars in one call silently failed to escalate the
     score range at all (confirmed by reproducing it live: added 150 cars to an
     8-car show, range stayed at 5). One-line fix: `db.flush()` between the car
     loop and `check_and_escalate()`. Re-verified live after the fix — adding one
     more car correctly escalated `score_range_max` 5 → 10, bumped
     `configuration_revision`, and the very next `/api/v1/sync` response reflected
     the new range, the exact condition `scoring_updated.js`'s notice fires on.
     Unrelated to the simulator itself, but genuinely blocking verification of
     this task's own item-2 fidelity target for the Scoring Updated notice.
  4. **Theme values are hardcoded from `firmware/src/ui/theme.cpp`, NOT reused
     from Home Base's own `tokens.css`.** Confirmed by reading both: they're
     different systems (Home Base's is its own single-theme web styling; the
     firmware's dual-theme Dark/Daylight hex values — especially Daylight, which
     `theme.cpp`'s own comment says isn't documented anywhere else — are specific
     to the physical device). Font files ARE reused from Home Base's existing
     `/static/fonts/*.woff2` (same vendored files, no duplication of binaries).
  5. **On-screen keyboard/keypad are the only default input path, by
     construction, not just by convention.** Every text/numeric field in
     `screens/*.js` routes through `keyboard.js`/`keypad.js`'s key-handler
     functions — there is no bare `<input>` a developer could type into instead.
     "Use physical keyboard" (Settings, default OFF — `screens/settings.js`) is
     the only thing that wires up real `keydown` events, and even then they call
     the exact same `applyKey()`/`finish()` functions a tap would, so behavior is
     identical either way — the on-screen path can't be silently bypassed.
  6. **Out of Range (task requirement 6) short-circuits `sync.js`'s `attempt()`
     at the same point a real WiFi scan miss would** — before any `fetch()` call,
     going straight into the backoff-doubling path (`applyMiss`). This makes
     queue-survives-a-miss and backoff-doubles-then-caps-at-900s behavior
     genuinely exercised through the real trigger-model code, not faked at a
     different layer.
  7. **What's verified and what isn't.** Verified for real, against a live
     server: every simulator JS/CSS/data asset resolves (200) from a running
     `uvicorn` instance; a real `POST /api/v1/sync` round trip (config/cars/
     summary all correctly shaped); a real scored-car submission accepted and
     matched against `results[].status`; a real range escalation (150+ cars)
     surfaced through to a sync response, including the bug fix in point 3 that
     was required to make that path work at all. NOT verified: actual rendering
     and interaction in a browser (this environment has no browser tool) — every
     screen/component file was written by porting the named firmware source
     file's logic and wording directly and re-read for correctness, the same
     "compiles and traces correctly against spec" ceiling every firmware task
     without a physical board has had, just applied to a browser instead of an
     ESP32 this time. The task's own Verification section's browser-dependent
     items (side-by-side visual comparison, physical-keyboard opt-in behavior,
...
## Open items — full backstory

Full-length sourcing for the Open entries kept in DECISIONS.md's own Open section (trimmed there to the essential blocking fact + next action). Also where a struck/reversed Open item's full original text lives, per DECISIONS.md's own "strike, don't delete" convention.
